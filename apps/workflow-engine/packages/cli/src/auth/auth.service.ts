	...
	createAuthMiddleware({
		...
		return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
			...
			if (authHeader?.startsWith('Bearer ')) {
				...
				try {
					// Hack to bypass n8n auth - verify Otto JWT from header
					const { jwtVerify } = require('jose');
					const JWT_SECRET = new TextEncoder().encode(process.env.JWT_SECRET || 'otto_super_secret_dev_key');
					const { payload } = await jwtVerify(token, JWT_SECRET);
					
					// Get the first user from the DB to assume their identity, or a specific system user
					...
					// Pass through to let it fail below
				}
			}
