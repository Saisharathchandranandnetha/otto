...
class DatabaseConfig(BaseSettings):
    ...
    def SQLALCHEMY_ENGINE_OPTIONS(self) -> SQLAlchemyEngineOptionsDict:
        ...
        if self.SQLALCHEMY_DATABASE_URI_SCHEME.startswith("postgresql"):
            ...
            schema_opt = "-c search_path=ai_engine"
            if session_timezone_override:
                timezone_opt = f"-c timezone={session_timezone_override}"
                merged_options = f"{merged_options} {timezone_opt} {schema_opt}".strip() if merged_options else f"{timezone_opt} {schema_opt}"
            else:
                merged_options = f"{merged_options} {schema_opt}".strip() if merged_options else schema_opt
            if merged_options:
