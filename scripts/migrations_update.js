...
const dbDir = path.join(root, 'apps', 'web', 'db', 'migrations');
const pkgMigrationsDir = path.join(root, 'packages', 'database', 'migrations');

// Delete web migrations entirely
if (fs.existsSync(dbDir)) {
  fs.rmSync(dbDir, { recursive: true, force: true });
  console.log(`Deleted ${dbDir}`);
}

const multiTenancySql = `-- Migration 008: Multi-Tenancy & SSO (Otto Core)
...
const flexActionTypesSql = `-- Migration 011: Flex Action Types
ALTER TABLE actions DROP CONSTRAINT IF EXISTS actions_type_check;
`;

const agentsSql = `-- Migration 012: Agents
...
const workflowsSql = `-- Migration 013: Workflows
...
fs.writeFileSync(path.join(pkgMigrationsDir, '008_multi_tenancy.sql'), multiTenancySql);
fs.writeFileSync(path.join(pkgMigrationsDir, '011_flex_action_types.sql'), flexActionTypesSql);
fs.writeFileSync(path.join(pkgMigrationsDir, '012_agents.sql'), agentsSql);
fs.writeFileSync(path.join(pkgMigrationsDir, '013_workflows.sql'), workflowsSql);

console.log('Migrations rewritten successfully.');
